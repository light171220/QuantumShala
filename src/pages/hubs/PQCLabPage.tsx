...
import { KeyViewer, BenchmarkChart, LearningCenterTab, ThreatTimeline, ComparisonDashboard, ParameterDetails, KeyExporter, TLSHandshakeDemo, CertificateChainDemo, AlgorithmFlowDiagram, LatticeVisualizer } from '@/components/pqc'
import { MLKEM, type MLKEMKeyPair } from '@/lib/pqc/kem/ml-kem'
import { MLDSA, type MLDSAKeyPair } from '@/lib/pqc/dsa/ml-dsa'
import { SLHDSA, type SLHDSAKeyPair } from '@/lib/pqc/dsa/slh-dsa'
...
import { usePQCLearningStore } from '@/stores/pqcLearningStore'
...
const getSLHVariant = (variantStr: string): 'SLH-DSA-128f' | 'SLH-DSA-128s' | 'SLH-DSA-192f' | 'SLH-DSA-192s' | 'SLH-DSA-256f' | 'SLH-DSA-256s' => {
  const map: Record<string, 'SLH-DSA-128f' | 'SLH-DSA-128s' | 'SLH-DSA-192f' | 'SLH-DSA-192s' | 'SLH-DSA-256f' | 'SLH-DSA-256s'> = {
    '128f': 'SLH-DSA-128f',
    '128s': 'SLH-DSA-128s',
    '192f': 'SLH-DSA-192f',
    '192s': 'SLH-DSA-192s',
  ...
  return map[variantStr] || 'SLH-DSA-128f'
}
...
export default function PQCLabPage() {
  ...
  const [slhKeyPair, setSlhKeyPair] = useState<SLHDSAKeyPair | null>(null)
  ...
  const { trackKeyGeneration, trackEncapsulation, trackSignature } = usePQCLearningStore()
  ...
  const handleGenerateKeys = useCallback(async () => {
    ...
    try {
      if (selectedAlgo.type === 'KEM') {
        const kem = new MLKEM(getKEMVariant(selectedAlgo.variant as number))
        const keyPair = kem.keyGen()
        setKemKeyPair(keyPair)
        setDsaKeyPair(null)
        setSlhKeyPair(null)
        trackKeyGeneration('ml-kem')
      } else if (selectedAlgo.type === 'Signature') {
        const dsa = new MLDSA(getDSAVariant(selectedAlgo.variant as number))
        const keyPair = dsa.keyGen()
        setDsaKeyPair(keyPair)
        setKemKeyPair(null)
        setSlhKeyPair(null)
        trackKeyGeneration('ml-dsa')
      } else if (selectedAlgo.type === 'Hash-Signature') {
        const slh = new SLHDSA(getSLHVariant(selectedAlgo.variant as string))
        const keyPair = slh.keyGen()
        setSlhKeyPair(keyPair)
        setKemKeyPair(null)
        setDsaKeyPair(null)
        trackKeyGeneration('slh-dsa')
      }
    } catch (error) {
  ...
  }, [selectedAlgo, trackKeyGeneration])
  ...
  }, [kemKeyPair, selectedAlgo, trackEncapsulation])
  ...
  const handleSign = useCallback(async () => {
    const activeKeyPair = dsaKeyPair || slhKeyPair
    ...
    try {
      ...
      if (selectedAlgo.type === 'Signature' && dsaKeyPair) {
        const dsa = new MLDSA(getDSAVariant(selectedAlgo.variant as number))
        const result = dsa.sign(message, dsaKeyPair.secretKey)
        setSignature(result.signature)
        trackSignature()
      } else if (selectedAlgo.type === 'Hash-Signature' && slhKeyPair) {
        const slh = new SLHDSA(getSLHVariant(selectedAlgo.variant as string))
        const result = slh.sign(message, slhKeyPair.secretKey)
        setSignature(result.signature)
        trackSignature()
      }
      setVerificationResult(null)
    ...
    setIsSigning(false)
  }, [dsaKeyPair, slhKeyPair, plaintext, selectedAlgo, trackSignature])

  const handleVerify = useCallback(async () => {
    const activeKeyPair = dsaKeyPair || slhKeyPair
    if (!activeKeyPair || !signature) return
    ...
    try {
      ...
      if (selectedAlgo.type === 'Signature' && dsaKeyPair) {
        ...
        const slh = new SLHDSA(getSLHVariant(selectedAlgo.variant as string))
        const valid = slh.verify(message, signature, slhKeyPair.publicKey)
        setVerificationResult(valid)
      }
    } catch (error) {
  ...
  }, [dsaKeyPair, slhKeyPair, signature, plaintext, selectedAlgo])
  ...
  const handleRunBenchmarks = useCallback(async () => {
    ...
    for (const algo of PQC_ALGORITHMS) {
      ...
      if (algo.type === 'KEM') {
        const kem = new MLKEM(getKEMVariant(algo.variant as number))
        ...
        const dsa = new MLDSA(getDSAVariant(algo.variant as number))
      ...
      } else if (algo.type === 'Hash-Signature') {
        const slh = new SLHDSA(getSLHVariant(algo.variant as string))
        ...
        const signStart = performance.now()
        const sigResult = slh.sign(message, keyPair.secretKey)
        const signTime = performance.now() - signStart

        const verifyStart = performance.now()
        slh.verify(message, sigResult.signature, keyPair.publicKey)
        const verifyTime = performance.now() - verifyStart
  ...
  const currentKeyPair = selectedAlgo.type === 'KEM' ? kemKeyPair : selectedAlgo.type === 'Signature' ? dsaKeyPair : slhKeyPair
      ...
      <Tabs value={selectedTab} onChange={setSelectedTab}>
        <div className="overflow-x-auto -mx-4 px-4 md:mx-0 md:px-0">
          <TabsList className="w-max md:w-auto">
            ...
            <TabsTrigger value="protocols" className="text-xs md:text-sm">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
            ...
            <div className="lg:col-span-8 space-y-4 order-1 lg:order-2">
              <Card variant="neumorph" className="p-4">
                  ...
                  {selectedAlgo.type === 'KEM' ? (
                    ...
                    <>
                      <Button
                        variant="secondary"
                        onClick={handleSign}
                        disabled={!dsaKeyPair && !slhKeyPair}
                        isLoading={isSigning}
                        size="sm"
                      >
                ...
                {(selectedAlgo.type === 'Signature' || selectedAlgo.type === 'Hash-Signature') && (
                  <div className="mb-4">
                ...
                {currentKeyPair && (
                  <KeyViewer
                    keyPair={{
                      ...
                      algorithm: selectedAlgo.type === 'KEM' ? 'ml-kem' : selectedAlgo.type === 'Signature' ? 'ml-dsa' : 'slh-dsa',
        ...
        <TabsContent value="protocols" className="mt-4 md:mt-6">
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card variant="neumorph" className="p-4">
                <CardHeader className="p-0 pb-4">
                  ...
                  <CardDescription className="text-xs">
                    Interactive demonstration of post-quantum key exchange in TLS
                  </CardDescription>
                </CardHeader>
                <TLSHandshakeDemo />
              </Card>
              ...
              <Card variant="neumorph" className="p-4">
                <CardHeader className="p-0 pb-4">
                  ...
                  <CardDescription className="text-xs">
                    X.509 certificate chain with ML-DSA signatures
                  </CardDescription>
                </CardHeader>
            ...
            <Card variant="neumorph" className="p-4">
              <CardHeader className="p-0 pb-4">
                ...
                <CardDescription className="text-xs">
                  Key milestones and deadlines for post-quantum migration
                </CardDescription>
              </CardHeader>
              <ThreatTimeline />
            </Card>
        ...
        <TabsContent value="compare" className="mt-4 md:mt-6">
          <div className="space-y-6">
            <Card variant="neumorph" className="p-4">
              <CardHeader className="p-0 pb-4">
                <CardTitle className="text-base flex items-center gap-2">
                ...
                <CardDescription className="text-xs">
                  Compare classical and post-quantum algorithms side-by-side
                </CardDescription>
              </CardHeader>
              <ComparisonDashboard />
            </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <ParameterDetails algorithm={selectedAlgo.type} variant={selectedAlgo.name} />
              {currentKeyPair && (
                <KeyExporter
                  publicKey={currentKeyPair.publicKey}
                  secretKey={currentKeyPair.secretKey}
                  algorithm={selectedAlgo.type}
                  variant={selectedAlgo.name}
                />
              )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card variant="neumorph" className="p-4">
                ...
                <AlgorithmFlowDiagram
                  algorithm={selectedAlgo.type === 'KEM' ? 'ml-kem' : selectedAlgo.type === 'Signature' ? 'ml-dsa' : 'slh-dsa'}
                  operation={selectedAlgo.type === 'KEM' ? 'encrypt' : 'sign'}
                />
              </Card>
              ...
              <Card variant="neumorph" className="p-4">
                <CardHeader className="p-0 pb-4">
                  ...
                  <CardDescription className="text-xs">
                    2D visualization of lattice-based cryptography concepts
                  </CardDescription>
                </CardHeader>
                <LatticeVisualizer dimensions={2} showSVP showLWE latticeSize={5} />
              </Card>
            </div>
          </div>
        </TabsContent>

        <TabsContent value="learn" className="mt-4 md:mt-6">
          <LearningCenterTab />
        </TabsContent>
      </Tabs>
