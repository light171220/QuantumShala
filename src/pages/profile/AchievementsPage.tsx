...
export default function AchievementsPage() {
  ...
  const achievements = ACHIEVEMENT_DEFINITIONS.map(def => {
    ...
    // Progress is now tracked via userAchievement.progress from LearningProgress
    if (userGameStats && def.target && def.category === 'streak') {
      progress = Math.min(100, ((userGameStats.currentStreak || 0) / def.target) * 100)
    }
    // Note: Other progress stats (lessonsCompleted, quizzesPassed, circuitsCreated)
    // should be derived from LearningProgress and Circuit models
