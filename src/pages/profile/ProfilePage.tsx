...
export default function ProfilePage() {
  ...
  // Progress stats derived from LearningProgress
  const lessonsCompleted = Object.values(localProgress).filter(p => p.status === 'completed').length
  const quizzesPassed = Object.values(localProgress).filter(
    p => p.bestQuizScore !== undefined && p.bestQuizScore >= 70
  ).length
  const totalTimeMinutes = Object.values(localProgress).reduce((acc, p) => acc + (p.timeSpentMinutes || 0), 0)
  const circuitsCreated = 0 // TODO: Derive from Circuit model

  const totalXp = userGameStats?.totalXp || 0
  const level = userGameStats?.level || 1
  const currentStreak = userGameStats?.currentStreak || 0
  const longestStreak = userGameStats?.longestStreak || 0
