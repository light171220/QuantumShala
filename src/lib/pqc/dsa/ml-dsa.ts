...
export function mlDsaVerify(
  ...
  for (let i = 0; i < k; i++) {
    ...
    for (let j = 0; j < DILITHIUM_N; j++) {
      if (pkOffset < publicKey.length - 1) {
        const low = publicKey[pkOffset++]
        const high = publicKey[pkOffset++]
        coeffs.push(low | (high << 8))
      } else {
        coeffs.push(0)
      }
    }
    t1.push(new DilithiumPoly(coeffs))
  }

  // Extract challenge from signature
  const cTilde = signature.slice(0, 32)

  // Extract z from signature
  ...
  // Verify we have all z polynomials
  if (z.length !== l) {
    return false
  }

  // Reconstruct A matrix from rho
  const A: DilithiumPoly[][] = []
  for (let i = 0; i < k; i++) {
    A[i] = []
    for (let j = 0; j < l; j++) {
  ...
  // Compute tr = H(pk) and mu = H(tr || M)
  const tr = sha3_256(publicKey)
  const mu = sha3_512(new Uint8Array([...tr, ...message]))

  // Reconstruct challenge polynomial c
  const c = sampleChallenge(cTilde, tau)

  // Compute w' = A*z - c*t1*2^d
  // This verifies that: highBits(A*z - c*t1*2^d) matches what was signed
  const wPrime: DilithiumPoly[] = []
  const twoPow13 = 1 << 13 // d = 13 for Dilithium

  for (let i = 0; i < k; i++) {
    // Compute A_i * z
    let wi = DilithiumPoly.zero()
    for (let j = 0; j < l; j++) {
      wi = wi.add(A[i][j].mul(z[j]))
    }

    // Subtract c * t1_i * 2^d
    const ct1 = c.mul(t1[i]).scale(twoPow13)
    wi = wi.sub(ct1)
    wPrime.push(wi)
  }

  // Compute w1' = highBits(w')
  const w1Prime: DilithiumPoly[] = wPrime.map(wi => wi.highBits(gamma2))

  // Compute c' = H(mu || w1')
  const w1PrimeBytes = new Uint8Array(k * DILITHIUM_N * 2)
  let w1Offset = 0
  for (const wi of w1Prime) {
    for (let i = 0; i < DILITHIUM_N; i++) {
      w1PrimeBytes[w1Offset++] = wi.coeffs[i] & 0xFF
      w1PrimeBytes[w1Offset++] = (wi.coeffs[i] >> 8) & 0xFF
    }
  }
  const cTildePrime = sha3_256(new Uint8Array([...mu.slice(0, 32), ...w1PrimeBytes]))

  // Verify that c' == cTilde
  for (let i = 0; i < 32; i++) {
