...
// Note: Progress stats (lessonsCompleted, quizzesPassed, circuitsCreated, totalTimeMinutes)
// are now derived from LearningProgress model instead of being stored in UserProfile

// Current user ID for service-level operations (unused since Amplify handles auth automatically)
...
export async function completeLesson(lessonId: string): Promise<void> {
  try {
    ...
    if (progress && progress.length > 0) {
      ...
      // Stats are now derived from LearningProgress, no need to update UserProfile
    }
  } catch (error) {
