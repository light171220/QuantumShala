...
export async function awardXP(
  ...
  try {
    ...
    if (!stats || stats.length === 0) {
      throw new Error('User game stats not found')
    }
...
export async function updateStreak(userId: string): Promise<{ currentStreak: number; isNewDay: boolean }> {
  try {
    ...
    const userStats = stats[0]
    const lastActive = userStats.lastActiveAt ? new Date(userStats.lastActiveAt) : null
    const now = new Date()
...
export async function getLeaderboard(
  ...
  try {
    ...
    const sorted = [...stats].sort((a, b) => (b.totalXp || 0) - (a.totalXp || 0))

    return sorted.map((s, index) => ({
      userId: s.userId,
      username: s.displayName, // Username is in Cognito, use displayName as fallback
      displayName: s.displayName,
      avatar: s.avatar ?? undefined,
      xp: s.totalXp || 0,
      rank: index + 1,
      lessonsCompleted: 0, // Derived from LearningProgress
