  ...
  persist(
    (set, get) => ({
      ...
      register: async (email: string, password: string, username: string, displayName?: string) => {
        ...
        try {
          ...
          if (result.isSignUpComplete) {
            const user = await authGetCurrentUser()
            if (user) {
              // Create UserGameStats for new user
              await createUserGameStats(user.id, {
      ...
      updateProfile: async (data) => {
        ...
        set({ isLoading: true, error: null })
        try {
          await updateUserGameStats(user.id, data)

          const updatedStats = await getUserGameStats(user.id)
          if (updatedStats) {
            set({
              user: {
                ...user,
                displayName: updatedStats.displayName || user.displayName,
                avatar: updatedStats.avatar || user.avatar,
              },
              isLoading: false,
            })
          } else {
