...
export const handler: Handler = async (event) => {
  ...
  console.log(`[VQE-${TIER.toUpperCase()}] Memory: ${Math.round(process.memoryUsage().heapUsed / 1024 / 1024)} MB`)
    ...
    const response = await runVQE(request, {
      ...
      onProgress: (iteration, energy) => {
        if (iteration % 5 === 0) {
          ...
          console.log(`[VQE-${TIER.toUpperCase()}] Iteration ${iteration}: E = ${energy.toFixed(8)} Ha, Mem: ${memUsed} MB`)
        }
      },
    })

    console.log(`[VQE-${TIER.toUpperCase()}] Final memory: ${Math.round(process.memoryUsage().heapUsed / 1024 / 1024)} MB`)
