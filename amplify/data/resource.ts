...
const schema = a.schema({
  // UserGameStats stores gamification data for users
  // Identity data (email, username, verified status) comes from Cognito
  // Settings are stored in localStorage or Cognito custom attributes
  UserGameStats: a
    .model({
  ...
  // Note: Track, Module, Lesson, and Quiz content models have been removed.
  // Content is now loaded directly from S3/CDN via src/services/content.ts
  // User progress is still tracked via LearningProgress and TrackProgress models above.
